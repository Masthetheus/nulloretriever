...
# Teste simples do pipeline corrigido
from workflow.scripts.python.funcoes_mestrado import *
...
# Pega um genoma de teste
genomas = glob.glob("/home/leveduras/integranteslab/matheus/Mestradoteste/Scripts/nullomer_c_trie_bit/workflow/data/genomes/GCA_*")
if not genomas:
    print("Nenhum genoma encontrado para teste!")
    sys.exit(1)

genoma = genomas[0]
output = "/tmp/teste_nullomers.txt"

print(f"Testando pipeline com:")
...
proc.wait()
print(f"DEBUG: Programa C terminou com código {proc.returncode}")
...
if os.path.exists(output):
    with open(output, 'r') as f:
        linhas = f.readlines()
    print(f"Arquivo de saída criado com {len(linhas)} linhas")
    if len(linhas) > 0:
