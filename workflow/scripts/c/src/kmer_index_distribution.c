...
int main(int argc, char *argv[]) {
        ...
        while (fgets(line, sizeof(line), f)) {
                ...
                if (line[0] == '>') {
                        ...
                        if (seqlen > 0) {
                                fprintf(stderr,
                                        "DEBUG: Starting kmer processing at "
                                        "line %d: %.50s\n",
                                        line_count, line);
                                ...
                                char *revcomp_seq =
                                    generate_revcomp_seq(seqlen, seq);
                        ...
                        fprintf(stderr,
                                "DEBUG: Starting FASTA body read processing at "
                                "line %d: %.50s\n",
                                line_count, line);
                        seq = read_fasta_body(line, seq, &seqlen);
                }
        }
        ...
        if (seqlen > 0) {
                ...
                process_kmers(revcomp_seq, seqlen, k, seen, bytes_per_sequence,
                              &tot);
                free(revcomp_seq);
        }
